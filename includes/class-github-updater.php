<?php
/**
 * GitHub Plugin Updater for Distribution - Jetpack Social
 *
...
class Dist_Jetpack_Social_GitHub_Updater {
     ...
     * @param string $repo GitHub repository (e.g., 'nonatech-uk/wp-dist-jetpack-social')
    ...
    public function __construct($plugin_file, $repo, $current_version) {
        ...
        $this->cache_key = 'dist_jpsocial_github_update_' . md5($repo);
    ...
    public function plugin_info($result, $action, $args) {
        ...
        return (object) array(
            'name' => 'Distribution - Jetpack Social',
            'slug' => dirname($this->plugin_slug),
            ...
            'sections' => array(
                'description' => 'Integrates Jetpack Social (Publicize) with Parish Distribution for controlled Facebook sharing.',
    ...
    public function add_check_update_link($links) {
        $url = wp_nonce_url(admin_url('plugins.php?dist_jpsocial_check_update=1'), 'dist_jpsocial_check_update');
    ...
    public function handle_check_update() {
        if (empty($_GET['dist_jpsocial_check_update'])) {
            return;
        }

        if (!current_user_can('update_plugins') || !wp_verify_nonce($_GET['_wpnonce'], 'dist_jpsocial_check_update')) {
            return;
        }
